...
static int deadband(int value) {
    return (std::abs(value) < config::STICK_DEADBAND) ? 0 : value;
}

static float turnScale(int throttle) {
    float speed = std::abs(throttle) / 127.0f;
    return config::TURN_SCALE_MIN + (config::TURN_SCALE_MAX - config::TURN_SCALE_MIN) * speed;
}
...
static float rampTurn(float current, float target) {
    ...
    float maxStep = turningHarder ? config::TURN_RAMP_UP : config::TURN_RAMP_DOWN;
...
void driveControl() {
    ...
    int throttle = leftY;
    if (std::abs(leftY) < std::abs(leftX) * config::SIDEWAYS_ZONE) throttle = 0;

    currentTurn = rampTurn(currentTurn, turn * turnScale(throttle));
    drive::chassis.arcade(throttle, static_cast<int>(std::round(currentTurn)));
}
